...
def _default_mpl_opts():
    ...
    if [f for f in matplotlib.font_manager.findSystemFonts() if "Ubuntu" in f]:
        ...
        opts["font.family"] = "DejaVu Sans"  # very safe fallback
...
def _default_plot_xy_opts():
    """
    Return default keyword arguments for plot_xy(), plot_state_xy(), and
    plot_state_corr_xy().
...
def plot_xy(
    ...
    **kwargs,
    ...
    Parameters
    ----------
    class_obj:      PyToF object, does not need to be provided by the user,
                    see ClassToF.py.
    x, y:           Integer indices selecting which quantities to plot along x
                    and y axes.
    do_new_figure:  bool, whether to create a new figure, by default True.
    literature:     object, reference data object with attributes lis, rhois,
                    Pis, etc.
    inset_plot_xy:  object, definition of an inset subplot with limits and
                    axes configuration.
    colorbar:       object, defining a Matplotlib colorbar, with attributes
                    such as mappable, label, width, stretch, etc.
    **kwargs:       Additional keyword arguments to override default plot
                    settings.
    """

    # Apply default and user supplied options:
    mpl_opts = {**_default_mpl_opts(), **kwargs}
    ...
    xy[0] = np.append(xy[0], 0)
    xy[1] = np.append(xy[1], 0)
    xy[2] = np.append(xy[2], xy[2][-1])
    xy[3] = np.append(xy[3], xy[3][-1])
    ...
    if literature is not None:
        ...
        for i in range(len(literature.lis)):
            ...
            shell_m = (
                ...
                / 3
                * np.pi
                * (li[p] ** 3 - np.append(li[p][1:], 0) ** 3)
                * literature.rhois[i][p]
            )
            summed_m = np.array(
                [sum(shell_m[i:]) for i in range(len(shell_m))]
            )

            # Data for plotting:
            literature_xys.append(
                [
                    li[p] / li[p][0],
                    summed_m / summed_m[0],
                    literature.rhois[i][p] / 1000,
                    literature.Pis[i][p] / 1e5,
                ]
            )

    # Create new figure if needed:
    if do_new_figure:
        class_obj.fig, [class_obj.ax, class_obj.cax] = plt.subplots(
            1, 2, layout="constrained", gridspec_kw={"width_ratios": [1, 0.01]}
        )
        class_obj.cax.axis("off")
        class_obj.ax.set_xlabel(label_xy[x])
        class_obj.ax.set_ylabel(label_xy[y])
        ...
        if inset_plot_xy is not None:
            class_obj.ax_ins = inset_axes(
                class_obj.ax,
                width=inset_plot_xy.width,
                height=inset_plot_xy.height,
                loc=inset_plot_xy.loc,
                borderpad=inset_plot_xy.borderpad,
                bbox_to_anchor=inset_plot_xy.bbox_to_anchor,
                bbox_transform=class_obj.ax.transAxes,
                axes_kwargs={
                    "xlim": inset_plot_xy.xlim,
                    "ylim": inset_plot_xy.ylim,
                    "xscale": scale_xy[inset_plot_xy.x],
                    "yscale": scale_xy[inset_plot_xy.y],
                },
            )
            class_obj.ax_ins.set_xlabel(label_xy[inset_plot_xy.x])
            class_obj.ax_ins.set_ylabel(label_xy[inset_plot_xy.y])
    ...
    if literature is not None:
        for i in range(len(literature_xys)):
            ...
            class_obj.ax.plot(
                literature_xys[i][x],
                literature_xys[i][y],
                color=literature.colors[i],
                linestyle=literature.linestyles[i],
                label=literature.labels[i],
                zorder=literature.zorders[i],
            )

            # Do the plotting (inset plot):
            if inset_plot_xy is not None:
                class_obj.ax_ins.plot(
                    literature_xys[i][inset_plot_xy.x],
                    literature_xys[i][inset_plot_xy.y],
                    linestyle=literature.linestyles[i],
                    label=literature.labels[i],
                    zorder=literature.zorders[i],
                )
    ...
    if colorbar is not None:
        gs = class_obj.ax.get_subplotspec().get_gridspec()
        gs.set_width_ratios([1, colorbar.width])
        class_obj.cax.axis("on")
        class_obj.fig.set_size_inches(
            mpl_opts["figure.figsize"][0] * colorbar.stretch,
            mpl_opts["figure.figsize"][1],
            forward=True,
        )

        cbar = class_obj.fig.colorbar(
        ...
        if hasattr(colorbar, "tick_lab"):
            ...
            cbar.set_ticklabels(colorbar.tick_lab)
    ...
    if opts["do_legend"]:
        class_obj.ax.legend(
            ...
            bbox_to_anchor=opts["bbox_to_anchor_legend"],
        )

    # Save the plot if needed:
    if opts["do_save"]:
        class_obj.fig.savefig(
            opts["path_name"] + "/" + opts["fig_name"] + "." + opts["format"],
            format=opts["format"],
...
def plot_shape(class_obj, **kwargs):
    ...
    **kwargs:       Additional keyword arguments to override default plot
    ...
    # Ensures a radius 0 entry is present:
    X[:N, :] = r_l_mu / np.max(class_obj.li)
    Y *= theta / (2 * np.pi) * 360
    Z[:N, :] = np.outer(
        class_obj.rhoi / np.max(class_obj.rhoi), np.ones_like(theta)
    )

    # Create new figure:
    fig, axs = plt.subplots(
        1, 2, layout="constrained", gridspec_kw={"width_ratios": [1, 0.07]}
    )

    # Do the plotting:
    con = axs[0].contourf(
        X, Y, Z, levels=opts["contourf_levels"], cmap=opts["contourf_cmap"]
    )
    ...
    line = axs[0].plot(
        ...
        label=r"average surface radius",
    )
    ...
    **kwargs:       Additional keyword arguments to override default plot
    ...
    for i in range(len(ss)):
        if i != 0:
            ...
            y = np.append(
                np.flip(ss[i] / np.max(abs(ss[i]))),
                ss[i][0] / np.max(abs(ss[i])),
            )

            # Do the plotting:
            val, exp = f"{np.max(abs(ss[i])):.1e}".split("e")
            class_obj.ax.plot(
                x,
                y,
            )
...
def plot_state_xy(
    ...
    **kwargs,
    ...
    """
    Plot selected properties of several models
    (for example radius vs. density).
    ...
    x, y:           Integer indices selecting which quantities to plot along x
                    and y axes.
    state:          2D array of model states
                    (rows = realizations, columns = parameters).
    what_model:     {'baro', 'dens'}, specifies whether to use barotropic or
                    density cost function.
    literature:     object, reference data object with attributes lis, rhois,
                    Pis, etc.
    inset_plot_xy:  object, definition of an inset subplot with limits and
                    axes configuration.
    ...
    **kwargs:       Additional keyword arguments to override default plot
    ...
    if colorbar is not None:
        ...
        colors = colorbar.cmap(
            (colorbar.sort_by - min(colorbar.sort_by))
            / (max(colorbar.sort_by) - min(colorbar.sort_by))
        )[idxs]
    ...
    do_legend = opts["do_legend"]

    # Do the iteration:
    for i in idxs:
        # Call cost functions depending on what model was used:
        if what_model == "baro":
            costJs = class_obj.baro_cost_function(
                state[i, :], return_sum=False
            )

        elif what_model == "dens":
            costJs = class_obj.dens_cost_function(
                state[i, :], return_sum=False
            )
            ...
            raise KeyError(
                c.WARN
                + "Invalid keyword for what_model! Use 'baro' or 'dens'."
                + c.ENDC
            )
        ...
        if i == idxs[0] and len(idxs) == 1:
            opts["color"] = color
            opts["do_save"] = do_save
            do_new_figure_arg = True
            literature_arg = literature
            opts["do_legend"] = do_legend
        ...
        plot_xy(
            ...
            x,
            y,
            do_new_figure=do_new_figure_arg,
            literature=literature_arg,
            inset_plot_xy=inset_plot_xy,
            colorbar=colorbar,
            **opts,
        )


def plot_state_corr_xy(
    class_obj,
    x,
    y,
    state,
    ...
    **kwargs,
    ...
    Displays a scatter plot of two quantities (specified by x, y) with
    associated error bars, marginal histograms, and Spearman correlation
    coefficients.
    ...
    what_model:     {'baro', 'dens'}, specifies whether to use barotropic or
                    density cost function.
    sigma_lim:      float, number of standard deviations to limit
                    plotted data points, by default np.inf (no limit).
    literature:     object, reference data object with attributes lis, rhois,
                    Pis, etc.
    colorbar:       object, defining a Matplotlib colorbar, with attributes
                    such as mappable, label, width, stretch, etc.
    **kwargs:       Additional keyword arguments to override default plot
    ...
    Js_err_list = np.array(Js_err_list)

    # Create new figure:
    fig = plt.figure(
        figsize=(mpl_opts["figure.figsize"][0], mpl_opts["figure.figsize"][0]),
        layout="constrained",
    )
    gs = fig.add_gridspec(
        2,
        3,
        width_ratios=(4, 1.25, 0.01),
    ...
    # Apply the sigma_lim potentially supplied by the user, sigma_lim=np.inf
    # has no effect:
    mask_x = (x_array - np.average(x_array)) ** 2 / max(
        np.std(x_array) ** 2, np.spacing(np.average(x_array))
    ) <= sigma_lim**2
    mask_y = (y_array - np.average(y_array)) ** 2 / max(
        np.std(y_array) ** 2, np.spacing(np.average(y_array))
    ) <= sigma_lim**2
    ...
    y_err = y_err[mask]
    color_list = [
        color for color, m in zip(color_list, mask, strict=True) if m
    ]

    # Plot the data:
    for xa, ya, xe, ye, color, match in zip(
        x_array, y_array, x_err, y_err, color_list, match_list, strict=True
    ):
        if not match:
            xe = np.nan
            ye = np.nan
        ax.errorbar(xa, ya, xerr=xe, yerr=ye, color=color, fmt="o")
    ...
    ax_leg.legend(
        ...
        [
            r"$\rho=${:.2f}".format(
                scipy.stats.spearmanr(x_array, y_array, nan_policy="omit")[0]
            ),
            r"$p=${:.2f}".format(
                scipy.stats.spearmanr(x_array, y_array, nan_policy="omit")[1]
            ),
        ],
        handletextpad=0,
        handlelength=0,
    ...
    if (x < len(Js_list[0, :])) and (y < len(Js_list[0, :])):
        ax.errorbar(
            class_obj.opts["Target_Js"][x - 1] / 10**x_scale,
            class_obj.opts["Target_Js"][y - 1] / 10**y_scale,
            xerr=class_obj.opts["Sigma_Js"][x - 1] / 10**x_scale,
            yerr=class_obj.opts["Sigma_Js"][y - 1] / 10**y_scale,
            color="k",
            fmt="o",
            capsize=3.0,
            capthick=3.0,
    ...
    if colorbar is not None:
        ...
        gs.set_width_ratios([4, 1.25, colorbar.width])
        fig.set_size_inches(
            mpl_opts["figure.figsize"][0] * colorbar.stretch,
            mpl_opts["figure.figsize"][0],
            forward=True,
        )
    ...
    ax.set_xlim(
        (
            np.min(x_array)
            - mpl_opts["axes.xmargin"] * (np.max(x_array) - np.min(x_array)),
            np.max(x_array)
            + mpl_opts["axes.xmargin"] * (np.max(x_array) - np.min(x_array)),
        )
    )
    ax.set_ylim(
        (
            np.min(y_array)
            - mpl_opts["axes.ymargin"] * (np.max(y_array) - np.min(y_array)),
            np.max(y_array)
            + mpl_opts["axes.ymargin"] * (np.max(y_array) - np.min(y_array)),
        )
    )
    ...
    if opts["do_legend"]:
        ax_histx.legend(
            [p1, p2],
            [
                rf"$\mu=${np.average(x_array):.3f}",
                rf"$\sigma=${np.std(x_array):.0e}",
            ],
            handletextpad=0,
            handlelength=0,
            loc=opts["loc_legend"],
        ...
        ax_histy.legend(
            [p1, p2],
            [
                rf"$\mu=${np.average(y_array):.3f}",
                rf"$\sigma=${np.std(y_array):.0e}",
            ],
            handletextpad=0,
            handlelength=0,
            loc=opts["loc_legend"],
...
def plot_autocorr(autocorr, **kwargs):
    ...
    Parameters
    ----------
    autocorr:   Array of estimated autocorrelation times for increasing step
                numbers.
    **kwargs:   Additional keyword arguments to override default plot settings.
    ...
    # Plot the data:
    ax.plot(
        100 * np.arange(1, len(autocorr) + 1),
        np.arange(1, len(autocorr) + 1),
        "--",
        label=r"$\tau=N/100$",
        color="black",
    )
